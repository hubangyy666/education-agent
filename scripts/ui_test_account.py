...
from backend.db import User,Run,Progress,Mistake,TaskCard,engine,cache,password_hash
username='qa_browser_primary'
with Session(engine) as db:
    if len(sys.argv)>1 and sys.argv[1]=='cleanup':
        for cls in (Run,Progress,Mistake,TaskCard): db.execute(delete(cls).where(cls.username==username))
