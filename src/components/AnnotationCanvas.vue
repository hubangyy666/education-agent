...
const props = withDefaults(defineProps<{ question: any; modelValue: any; guide?: boolean; findOnly?: boolean; labelsVisible?: boolean; disabled?: boolean; standard?: any; standardAssist?: boolean }>(), { guide: false, findOnly: false, labelsVisible: true, disabled: false, standardAssist: false })
...
// Standard answers are read-only unless onboarding explicitly renders one as a retry aid.
const showingStandard = computed(() => props.standard !== undefined && props.standard !== null)
const readOnly = computed(() => props.disabled || (showingStandard.value && !props.standardAssist))
...
<template>
  <div class="annotation-component" @keydown="keydown">
    ...
    <div v-if="showingStandard && !standardAssist" class="standard-view-note" role="status"><Icon name="LockKeyhole" :size="15"/>正在查看标准答案 · 绿色虚线为标准标注，查看期间不能修改</div>
      ...
      <div ref="surface" class="annotation-surface" :class="{ finding: findOnly, readonly: readOnly, 'drawing-mode': tool === 'draw', 'select-mode': tool === 'select' }" tabindex="0" role="application" :aria-label="findOnly ? '点击图片中任务要求的目标' : readOnly ? '图像标注画布，只读' : '图像标注画布；画框工具新增标注，选择工具移动，八个控制点调整边界'" :style="{ aspectRatio: question.width + '/' + question.height, width: zoom * 100 + '%', maxWidth: zoom * 490 * question.width / question.height + 'px', minWidth: '0', margin: '0 auto' }" @pointermove="move" @pointerup="up" @pointercancel="cancelDrag" @lostpointercapture="cancelDrag">
        ...
        <svg viewBox="0 0 1000 1000" preserveAspectRatio="none" class="annotation-overlay" @pointerdown="down($event)">
          ...
          <template v-if="showingStandard && question.type === 'box'">
            <g v-for="(box, index) in standardBoxes" :key="index" class="standard-annotation" :class="{'standard-annotation-flash':standardAssist}">
          ...
          <polygon v-if="showingStandard && question.type === 'polygon'" class="standard-polygon" :class="{'standard-annotation-flash':standardAssist}" :points="standardPolygon"/>
        </svg>
