...
<template>
  <div class="app-shell" :class="{immersive}">
    <header v-if="!immersive" class="site-header"><div class="header-inner">
      ...
      <div class="header-actions"><RouterLink to="/profile" class="energy" aria-label="累计完成题数"><b>{{state.dashboard?.total_count || 0}}</b><Icon name="Zap"/></RouterLink><button class="icon-button" aria-label="打开菜单" @click="menuOpen=!menuOpen"><Icon name="Menu"/></button><div v-if="menuOpen" class="header-menu"><RouterLink to="/profile" @click="menuOpen=false"><Icon name="Settings2"/>学习设置</RouterLink><button @click="menuOpen=false;signOut()"><Icon name="LogOut"/>退出登录</button></div></div>
    </div></header>
