...
- **多种标注交互**：支持单选分类、文本实体、单目标/多目标矩形框、多边形标注，矩形框支持独立对象列表、逐框标签、移动、四边与四角缩放、键盘微调、撤销、单框删除和一键清空所有框。
- **分层训练机制**：每个能力包含 5 个课关、1 个岗关和 1 个赛关；课关逐题反馈，答错可自行选择重试或继续，标注题答对或连续答错 3 次后可主动查看标准答案；查看时锁定画布，再试一次清空作答与标准答案。岗关整包质检并支持返修，赛关由服务端限时结算。
- **个人错题本**：正式训练中的错题自动收集到“我的”页面，可独立重做或主动移出；错题复答不改变正式技能分。
...
- **AI 学习导师**：首页与课关题内提供“小基”文字导师，可按需读取学情、检索知识和解释已提交结果；首页回答相关技能问题后附具体关卡推荐，点击即可进入训练；岗关、赛关不显示小基助手。
- **可追溯内容更新**：固定 V1 离线题库保证首次启动；题目工厂支持生成、独立审核、去重、原子发布和旧 Run 快照保留。
- **分层本地鼓励**：普通答对播放随项目提供的 Kokoro 中文 WAV；标注 IoU 严格超过 90% 时播放白色圆底 Lottie 绿对钩与新下载的 1.53 秒 CC0 真人男声“Congratulations!”，与普通鼓励女声区分。AI 回复和教学讲解不朗读，也不调用浏览器文字转语音。
- **完整成就册**：“我的成长”提供 17 枚课程、岗位、竞赛、质量、复盘与连续学习成就，可在弹窗中查看已点亮和未点亮进度。
...
| 关卡类型 | 数量 | 题量 | 反馈方式 |
| --- | ---: | ---: | --- |
| 课关 | 5 | 每关 7 题（约束为 6–8） | 逐题确定性反馈，答错可重试或继续；标框题答对也可清空后重练 |
| 岗关 | 1 | 20 题 | 整包提交后生成报告，可返修错误样本 |
| 赛关 | 1 | 10 题 | 10 分钟服务端计时，结束后统一反馈 |

仓库内 `data/initial-question-sets.json` 固定提供 10 个能力、70 个关卡、650 道 V1 题，当前题型分布为 215 道选择题、358 道矩形框、51 道多边形和 26 道文本实体，其中 426 道带图片、409 道为图片标注题。每个课关前至少 2 道、每个岗关前至少 10 道均为图片标注题，每个赛关的 10 道题全部为图片标注题；选择题、实体题等其他题型统一排在图片标注题之后。A1 的 65 题使用 30 张互不重复的图片和 35 道按技能编写的情境选择题，分别对应数据与标签、标注任务类型、标准答案、工作流程和数据安全。

### 3. 技能分

```text
技能分 = 加权答对证据 ÷ 加权作答证据 × 10
权重：课关 × 1，岗关 × 2，赛关 × 3
```

技能分范围为 `0.00–10.00`，保留两位小数。它与关卡完成数、入门诊断分、百分制训练报告和历史最高成绩分别统计：

- 课关每次提交都计入技能证据，因此每一次答错都会影响技能分，之后改对不会抹掉此前错答；
- 课关提交后立即更新；岗关每题按课关的 2 倍计入，赛关每题按 3 倍计入；
- 岗关和赛关在整包结算后更新，避免中途泄露对错；
- 返修中自动继承的正确答案不会重复累计。
- 错题本复答使用独立复习记录，不创建正式训练 Run，也不计入技能分或关卡成绩。
...
首次启动在某个能力没有活动题集时导入其 V1；若活动题集仍是旧的 5 题课关，或图片标注题数量与顺序不符合上述规则，则尽量保留原有合格内容，用固定题库补足并重新排序后原子发布新版本。既有历史 Run 快照不会改变。能力页“更新题目”会执行储备检查、技能均衡、必要的有界补充、生成与独立审核、答案执行校验、内容去重和事务发布。

A1 活动题集若含跨关重复图片、近似重复题或本次复核排除的错配题，启动时发布修订题集；已修正的题集不会反复升级。A1 刷新也排除 `data/a1-question-review.json` 中记录的 31 道旧题，并检查全套图片和题目去重；其他能力题库不受此修订影响。历史训练继续使用原快照，新开始的训练使用新题集。
...
2026-09-13 按当前工作树复验：前端生产构建通过，后端 **824 项测试通过**（2 条依赖弃用警告），语音文件、精准动画与异步交互 **21 项检查通过**；标注编辑器 **12 组检查通过**，答案查看与重试流程 **10 项检查通过**。
...
- [标注工具与标准答案验收](docs/annotation-updates-acceptance.md)
- [首页 AI 相关关卡推荐验收](docs/homepage-tutor-recommendations-acceptance.md)
